...
class takeoff implements rulesInterface {
    ...
    private function build(){
        ...
        $this->output .= '<div class="flex flex-col gap-4 font-thin ">';
        $this->output .= $this->takeoff();
    ...
    private function momentum(){
        $out = '<div class="flex flex-row gap-4">';
            ...
            $out .= '<div class ="w-3/4">';
                $out .= 'Your momentum should be <span>' . $this->config[__FUNCTION__] . '</span> once you have taken off.';
            $out .= '</div>';
        $out .= '</div>';
    return $out;    }
