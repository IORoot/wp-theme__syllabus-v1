...
class duration implements rulesInterface {
    ...
    private function duration(){
        $out = '<h5 class="capitalize text-2xl">'.__FUNCTION__.' Rule.</h5>';
            $out .= '<div class ="w-full font-light">';
                $out .= 'The '.__FUNCTION__.' must be <span class="text-emerald-500 font-medium">'. $this->config[__FUNCTION__] . '</span> ';
            $out .= '</div>';
        return $out;
    }
    ...
    private function more_or_less(){
        $out = '<div class="flex flex-row gap-4">';
            ...
            $out .= '<div class ="w-3/4">';
                $out .= 'The duration can be '.$this->config[__FUNCTION__].' than specified.';
            $out .= '</div>';
        $out .= '</div>';
        return $out;    }
